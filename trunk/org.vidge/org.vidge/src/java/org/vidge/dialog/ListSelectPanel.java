...
public class ListSelectPanel<F> {
	...
	private SelectionAdapter selector;
	...
	public void setSelection(F selection) {
		if (selection != null) {
			table.removeSelectionListener(selector);
			...
			table.addSelectionListener(selector);
		}
	}
	...
	private void createContentSingle(Composite parent, Class<F> klass, List<F> list, F value) {
		...
		section.setText(propertyExplorer == null ? StringUtil.NN : propertyExplorer.getValidator().getHelp());
	...
	private class SingleSelector extends SelectionAdapter {
	...
	private class Selector extends SelectionAdapter {
