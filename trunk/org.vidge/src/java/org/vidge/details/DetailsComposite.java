...
public class DetailsComposite extends Composite {
	...
	public DetailsComposite(Composite parent, int style) {
		...
		if ((style & NO_HEADER) == 0) {
			title = toolkit.createLabel(this, "", SWT.WRAP); //$NON-NLS-1$
	...
	private void buildProperties(Details details1, Composite parent) {
		for (final DetailProperty property : details1.getProperties()) {
			if (property.getValue() == null) {
				...
				Label text = toolkit.createLabel(parent, "", SWT.WRAP);
				...
				case IMAGE:
					...
					if (property.getValue() instanceof Image) {
						...
						hyperlink.addHyperlinkListener(new IHyperlinkListener() {
							...
							@Override
							public void linkActivated(HyperlinkEvent e) {
								RawResourceDialog dialog = new RawResourceDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Resource", value.getImageData().data, 0);
								dialog.open();
							}
						});
					} else {
						...
						hyperlink.addHyperlinkListener(new IHyperlinkListener() {
							...
							@Override
							public void linkActivated(HyperlinkEvent e) {
								RawResourceDialog dialog = new RawResourceDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Resource", value, 0);
								dialog.open();
							}
						});
					}
