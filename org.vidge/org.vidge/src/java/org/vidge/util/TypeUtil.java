...
public class TypeUtil {
	...
	private static boolean isTextControl(Class<?> klass) {
		if (klass != null) {
			if (klass.equals(String.class) || klass.equals(Integer.class) || klass.equals(Long.class) || klass.equals(Character.class)
				...
				|| klass.equals(Byte.class) || klass.equals(int.class) || klass.equals(double.class) || klass.equals(float.class) || klass.equals(long.class)
				|| klass.equals(char.class) || klass.equals(byte.class) || klass.isEnum()) {
				return true;
			}
		}
		return false;
		...
		if (entityExplorer == null) {
			if (VidgeSettings.getUseObjectExplorer()) {
				if (propertyExplorer != null) {
					...
					throw new VidgeException("***2 Not found explorer for " + inputClass);
				}
			} else {
				throw new VidgeException("*** Not found explorer for " + inputClass);
			}
		}
	...
	public static Class<?> getGenericClass(Class<?> clazz, Type genericReturnType) {
		String className = genericReturnType + StringUtil.NN;
		if (!className.contains("?")) {
			if (className.contains("<") && className.contains(">")) {
				...
				className = className.replaceAll("class", StringUtil.NN).replaceAll(";", StringUtil.NN).trim();
