...
public class FormExplorer<T> extends EntityExplorer {
	...
	protected void loadOutherProps(List<IPropertyExplorer> list) {
		try {
			CommonProperties properties = new CommonProperties("form_" + Locale.getDefault().getLanguage() + PROPERTIES2);
			if (properties.isLoaded()) {
				String keyTemplate = form.getClass().getSimpleName() + StringUtil.DOT + PNAME + StringUtil.DOT + LABEL;
				for (IPropertyExplorer explorer : list) {
					String key = keyTemplate.replace(PNAME, explorer.getPropertyName());
					String string = properties.getPropertyMap().get(key);
					if (string != null) {
						String[] split = string.split(StringUtil.SEMICOLON);
						PropertyExplorer explorer2 = (PropertyExplorer) explorer;
						if (split.length > 0)
							explorer2.setLabel(split[0]);
						if (split.length > 1)
							explorer2.setOrder(Double.valueOf(split[1]));
						if (split.length > 2)
							explorer2.setTotal(split[2].equals("1"));
						if (split.length > 3)
							explorer2.setAverage(split[3].equals("1"));
					}
				}
			}
	...
	public Color getBackground(Object element) {
		if (IFormColor.class.isAssignableFrom(form.getClass())) {
			return ((IFormColor) form).getBackground(element);
		}
		return null;
	}
	...
	public Color getForeground(Object element) {
		if (IFormColor.class.isAssignableFrom(form.getClass())) {
			return ((IFormColor) form).getForeground(element);
		}
		return null;
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public IFormComparator<T> getComparator() {
		if (IFormComparator.class.isAssignableFrom(form.getClass())) {
			return (IFormComparator<T>) form;
		}
		return null;
	}
}
