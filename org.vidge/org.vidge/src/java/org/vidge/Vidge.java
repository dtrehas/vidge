...
public class Vidge {
	...
	public static final int ALLACTIONS = SWT.MAX << 10;
	...
	public static <T> TablePanel<T> createTable(Composite parent, Class<T> objClass, Class<?> formClass, List<T> objectList) {
		TablePanel tablePanel = new TablePanel(objClass, objectList, formClass, Vidge.ALLACTIONS);
		tablePanel.createViewer(parent);
		return tablePanel;
	}

	public static TablePanel createNotVirtualTable(Composite parent, Class<?> objClass, Class<?> formClass, List<?> objectList, int style) {
	...
	public static TablePanel createNotVirtualTableEx(Composite parent, Class<?> objClass, FormExplorer explorer, List<?> objectList, int style) {
		TablePanel tablePanel = new TablePanel(objClass, objectList, explorer, style | Vidge.NO_VIRTUAL | Vidge.NO_ACTIONS | Vidge.NO_TOOLKIT);
		tablePanel.createViewer(parent);
		return tablePanel;
	}
	...
	public static <T> void registerContextExplorer(String context, Class<? extends T> klass, IEntityExplorer explorer) {
		FormRegistry.registerContextExplorer(context, klass, explorer);
	}

	public static <T> void registerExplorer(Class<? extends T> klass, IEntityExplorer explorer) {
		FormRegistry.registerExplorer(klass, explorer);
	}
	...
	public static <T> SingleObjectDialog<T> createObjectDialog(IForm<T> form, String title) {
		return new SingleObjectDialog(new FormExplorer(form), title);
	}

	public static <T> ObjectListSelectDialog<T> createListDialog(String title, Class<T> klassIn, List<T> list, boolean singleSelect) {
		return new ObjectListSelectDialog(title, klassIn, list, singleSelect);
	}
